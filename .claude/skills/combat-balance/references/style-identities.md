...
Each style wins on a *different axis*. When re-ratcheting after a change, move the
...
|---|---|---|---|
| **AB** Aimed Blow | precision | Inherent **called shot**: DF-scaled armor-mitigation bypass + hit-location shift toward severity. | `[-13,-6,-10,-7,-5,+1]` — low ATT is the *cost* of precision; the bypass is the payoff. |
...
| **WS** Wall of Steel | anti-tempo brick | **Immovable**: negates LU/PL momentum payoffs aimed at it + a small attrition floor. | `[-4,-2,-9,0,-2,0]` — slowest style (low INI); denies tempo rather than gaining it. Weak: re-ratchet usually *lightens*. |
...
- **The matchup matrix** (`MATCHUP_MATRIX`) encodes the rock-paper-scissors between
  ...
  walls; WS breaks LU/PL snowballs). Keep it antisymmetric; it is *not* where a
