...
## 2024-11-23 - Flatten non-Vendetta opponent pool lookup in competition worker
**Learning:** `generateBoutBids` evaluated non-Vendetta target options during an inner loop processing every warrior in the active roster. Each iteration re-scanned the entire `rivals` array to find opponents and filter them by the "Active" status. This resulted in O(W * R * Opponents) work.
**Action:** Lift static collections that don`t change per-warrior out of hot loops. In this case, `nonVendettaOpponents` was constructed by filtering/flattening the opponents pool before entering the `activeRoster` loop, simplifying inner loop complexity and avoiding repeated array allocations.
