...
## 2024-05-17 - [Optimizing Regex Template Helpers]
**Learning:** The codebase has utility files replacing `{{tokens}}` in templates (`gazetteTemplateHelpers.ts`, `narrativeTemplateEngine.ts`, `narrativePBPUtils.ts`). When using chained or looping `.replace()` calls, especially with new RegExps per interpolation, it becomes a performance bottleneck for frequently invoked loops.
**Action:** Use a single-pass `replace` with a pre-compiled `RegExp` matching all templates using `/\{\{\s*([^{}\s]+)\s*\}\}/g` and passing a replacer callback. This was already partially done in some files by Bolt previously, but missing in `gazetteTemplateHelpers.ts` which uses `new RegExp` indirectly or needs further optimization. However, looking closer at `gazetteTemplateHelpers.ts`, it already uses `/\{\{\s*([^{}\s]+)\s*\}\}/g` inside `t()`.
## 2024-05-17 - [Optimizing Component Linkification]
**Learning:** `EventLog.tsx` creates a `new RegExp` combining all `entityNames` into a giant `pattern` inside `useMemo`, and runs `.split(pattern)` to linkify texts. Even with memoization, creating a massive OR-regex `(Name1|Name2|Name3...)` can be inefficient if many names are present, but it's executed per event (since `text` and `names` are inputs). A better approach in general, especially when there are many names (e.g. `allWarriorNames` has hundreds), is to compile the regex once globally or per-state, not per-event component.
**Action:** Review if `LinkifiedText` is causing performance issues by constructing the regex per component instead of sharing the regex. `names` is often `allWarriorNames`, which is shared but `text` changes, so the `useMemo` dependency array `[text, names]` means it recompiles the large regex for EVERY event!
## 2024-05-17 - [Optimizing EventLog RegEx Compilation]
**Learning:** `EventLog.tsx` creates a large OR-regex for linkifying names. Although `useMemo` is used, the dependencies are `[text, names]`. Since `text` is unique for every event title and subtitle, the component re-sorts, re-escapes, and re-compiles the regular expression for `allWarriorNames` (which can contain hundreds of names) thousands of times per render. This causes a massive CPU and garbage collection bottleneck.
**Action:** Extract the regex generation from the component and memoize it globally using a `WeakMap<string[], ...>`. Because `allWarriorNames` maintains the same array reference throughout a given render, the expensive regex compilation will run exactly once per array reference instead of $O(N_{events} \times 2)$ times.
