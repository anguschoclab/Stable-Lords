...
## 2026-06-28 - Replacing useWorldState with Narrow useShallow Slice
**Learning:** In Zustand stores, large full-state subscriptions like `useWorldState()` cause components to re-render anytime _any_ property of the global state updates (e.g. week advancing, treasury changes). Using `useShallow` with narrowed selectors prevents this and avoids O(N) wasteful re-renders on components.
**Action:** When extracting a narrow slice, explicitly declare what state fields the component depends on using `useGameStore(useShallow(s => ({ ... })))` instead of the global `useWorldState()`. Ensure the extraction does not involve inline object creations.
