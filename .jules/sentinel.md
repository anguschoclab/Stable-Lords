...
## 2024-10-24 - [CRITICAL] Remove unsafe-eval from Content Security Policy
**Vulnerability:** The application's Content Security Policy in `index.html` included `'unsafe-eval'` in the `script-src` directive.
**Learning:** Using `'unsafe-eval'` allows the execution of strings as code (e.g., via `eval()`, `setTimeout`, or `setInterval` with string arguments) and completely negates the primary protective benefit of CSP against Cross-Site Scripting (XSS). This is especially critical in Electron environments where an XSS might be leveraged for a sandbox escape.
**Prevention:** Avoid `'unsafe-eval'` in the `script-src` directive of CSP. Refactor code to avoid the use of `eval()` or similar functions. Modern build tools and frameworks generally do not require `'unsafe-eval'` in production.
