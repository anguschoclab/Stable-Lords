...
const masterTrainer: Trainer = {
  id: 'm', name: 'Master', tier: 'Master', focus: 'Mind',
  fame: 0, age: 45, contractWeeksLeft: 99,
...
function warrior(id: string, traits: string[], over: Partial<Warrior> = {}): Warrior {
  return {
    id, name: id, style: FightingStyle.SlashingAttack,
    styleName: FightingStyle.SlashingAttack,
    attributes: { WT: 12, WL: 12, ST: 12, SP: 12, DF: 12, AG: 12 },
    fame: 10, career: { wins: 2, losses: 6, kills: 0 }, status: 'Active', age: 26,
    traits, ...over,
  } as unknown as Warrior;
}
...
function methodicalRival(roster: Warrior[]): RivalStableData {
  return {
    id: 's1',
    owner: { id: 's1', name: 'Slow Sam', stableName: 'The Patient', fame: 0, renown: 0, titles: 0, personality: 'Methodical' },
    fame: 0, roster, treasury: 5000, ledger: [], trainingAssignments: [],
...
describe('liability cull honors the Release recommendation', () => {
  it('a 2-flaw warrior is released even by the most lenient personality', () => {
    ...
    const rival = methodicalRival([flawed, clean, warrior('a', []), warrior('b', []), warrior('c', [])]);
...
The Plan-2 harness asserts only that *some* traits/flaws exist. Now that class/Signature reachability and multi-flaw churn are fixed, tighten it so these regressions can't return.
...
- **Flaws still flow.** The soft cap gates only *positive development* attempts; births (`generateTraits`) and training botches can still push a warrior to 2 flaws, which Task 2 then churns.
