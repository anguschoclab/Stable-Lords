...
const DEBT_FLOOR = -800; const EMERGENCY_LOAN = 300;
```
...
## Risks
Over-correction into too-easy economy (keep numbers conservative; add a test that a *deliberately
idle* stable still trends negative); **rival-band drift** (stipend helps rivals too — if count
