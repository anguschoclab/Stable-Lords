...
function getSaveDirectory() {
    ...
    if (platform === 'darwin') {
        return path_1.default.join(os.homedir(), 'Library', 'Application Support', 'Stable Lords');
    }
    else if (platform === 'win32') {
        return path_1.default.join(process.env.APPDATA || '', 'Stable Lords');
    }
    else {
        return path_1.default.join(os.homedir(), '.stable-lords');
    }
}
...
function ensureSaveDirectory() {
    ...
    subdirs.forEach(dir => {
        const dirPath = path_1.default.join(saveDir, dir);
...
function createWindow() {
    mainWindow = new electron_1.BrowserWindow({
        ...
        x: store.get('windowBounds.x', undefined),
        y: store.get('windowBounds.y', undefined),
        minWidth: 1024,
        minHeight: 768,
        ...
        icon: path_1.default.join(__dirname, '../public/icons/icon-512.png'),
    });
    // Load the app
...
function createTray() {
    const iconPath = path_1.default.join(__dirname, '../public/icons/icon-192.png');
...
electron_1.ipcMain.handle('save-game', async (_event, slotId, state) => {
    try {
        ensureSaveDirectory();
        const filePath = path_1.default.join(getSaveDirectory(), 'hot_state', `${slotId}.json`);
...
electron_1.ipcMain.handle('list-saves', async () => {
    try {
        const hotStateDir = path_1.default.join(getSaveDirectory(), 'hot_state');
...
electron_1.ipcMain.handle('archive-bout-log', async (_event, year, season, boutId, logData) => {
    try {
        ensureSaveDirectory();
        const seasonDir = path_1.default.join(getSaveDirectory(), 'seasons', `season_${season}`, 'bouts');
        if (!fs.existsSync(seasonDir)) {
            fs.mkdirSync(seasonDir, { recursive: true });
        }
        const filePath = path_1.default.join(seasonDir, `${year}_${boutId}.json`);
...
electron_1.ipcMain.handle('retrieve-bout-log', async (_event, year, season, boutId) => {
    try {
        const filePath = path_1.default.join(getSaveDirectory(), 'seasons', `season_${season}`, 'bouts', `${year}_${boutId}.json`);
        if (!fs.existsSync(filePath)) {
...
electron_1.ipcMain.handle('archive-gazette', async (_event, season, week, markdown) => {
    try {
        ensureSaveDirectory();
        const seasonDir = path_1.default.join(getSaveDirectory(), 'seasons', `season_${season}`, 'gazettes');
        if (!fs.existsSync(seasonDir)) {
            fs.mkdirSync(seasonDir, { recursive: true });
        }
        const filePath = path_1.default.join(seasonDir, `week_${week}.md`);
...
electron_1.ipcMain.handle('retrieve-gazette', async (_event, season, week) => {
    try {
        const filePath = path_1.default.join(getSaveDirectory(), 'seasons', `season_${season}`, 'gazettes', `week_${week}.md`);
        if (!fs.existsSync(filePath)) {
