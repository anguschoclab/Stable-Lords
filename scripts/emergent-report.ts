/**
 * Emergent-behavior report harness — runs a long headless sim and prints a
 * deep-instrumentation report. Converted from `src/scripts/emergent-report.test.ts`
 * (a 33s non-test consumed by CI); now a standalone script.
 *
 * Run: `bun run scripts/emergent-report.ts`
 */
import { advanceWeek } from '../src/engine/pipeline/services/weekPipelineService';
import { drainDeferredBoutLogs } from '../src/engine/storage/deferredBoutLogs';
...
async function main() {
  ...
  const WEEKS = 52;
  const seed = 4242;
  let state: GameState = populateInitialWorld(createFreshState(seed.toString()), seed);
  ...
  if (state.week <= 0) {
    console.error('FAIL: sim never advanced');
    process.exit(1);
  }
  if ((state.deferredBoutLogs ?? []).length > 0) {
    console.error('FAIL: deferred bout logs left undrained');
    process.exit(1);
  }
  console.log(`OK: ${WEEKS} weeks simulated, week=${state.week}`);
}
