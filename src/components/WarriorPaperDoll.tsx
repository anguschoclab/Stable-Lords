    ...
    return (
      <Tooltip>
        ...
        <TooltipContent side="right" className="bg-black/90 border-arena-blood/40 text-xs p-2">
          <div className="font-bold uppercase tracking-widest text-[10px] mb-1 text-muted-foreground">{label}</div>
          {relevantInjuries.length > 0 ? (
            <div className="space-y-1">
              {relevantInjuries.map(i => (
                <div key={i.id} className="flex flex-col">
                  <span className={cn("font-bold text-arena-blood", i.severity === 'Minor' && 'text-arena-gold')}>
                    {i.severity} {i.name}
                  </span>
                  <span className="text-[9px] text-muted-foreground">{i.weeksRemaining} weeks left</span>
                </div>
              ))}
            </div>
          ) : (
    ...
    <div className={cn("relative flex items-center justify-center", className)}>
      ...
      >
        ...
        <Part 
          location="Abdomen" 
          label="Core & Vitals" 
          path="M38 56H62V75L50 85L38 75V56Z" 
        />
        ...
        <Part 
          location="Left Leg" 
          label="Left Leg" 
          path="M38 78L32 140L45 140L48 85L38 78Z" 
        />
        ...
        <Part 
          location="Right Leg" 
          label="Right Leg" 
          path="M62 78L68 140L55 140L52 85L62 78Z" 
        />
