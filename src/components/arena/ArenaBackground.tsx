...
const SEASON_MODIFIERS: Record<Season, { wallOverlay: string; accentColor: string; decorations: boolean }> = {
  spring: {
...
export default function ArenaBackground({ 
  ...
  return (
    <div className={cn("relative w-full h-full overflow-hidden", className)}>
      ...
      {/* Crowd silhouettes */}
      {style.crowd && (
        <CrowdLayer density={style.crowdDensity} tier={tier} />
      )}
...
function SeasonDecorations({ season, tier }: { season: Season; tier: ArenaTier }) {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      ...
      {tier !== "training" && (
        <>
          <div className="absolute top-4 left-8 text-2xl opacity-60">
            {icons[0]}
          </div>
          <div className="absolute top-4 right-8 text-2xl opacity-60">
            {icons[1]}
          </div>
  ...
  if (weatherLower.includes("blazing") || weatherLower.includes("sweltering")) {
    return (
      <div 
        ...
        style={{
          background: "linear-gradient(180deg, transparent 0%, rgba(255,200,100,0.1) 50%, transparent 100%)",
        }}
      />
    );
  }
