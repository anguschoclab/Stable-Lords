...
function getWeaponCategory(weaponName: string): "slash" | "bash" | "pierce" | "shield" | "fist" {
  ...
  if (w.includes("shield") || w.includes("small shield") || w.includes("medium shield") || w.includes("large shield")) return "shield";
  if (w.includes("sword") || w.includes("scimitar") || w.includes("axe") || w.includes("blade") || w.includes("scythe") || w.includes("halberd")) return "slash";
  if (w.includes("mace") || w.includes("hammer") || w.includes("flail") || w.includes("maul") || w.includes("morning star") || w.includes("morningstar")) return "bash";
  if (w.includes("spear") || w.includes("épée") || w.includes("epee") || w.includes("dagger") || w.includes("pike") || w.includes("lance")) return "pierce";
...
export default function ArenaFighter({
  ...
  const hasShield = shieldName && shieldName.toLowerCase() !== "none" && !shieldName.toLowerCase().includes("shield");
  const shieldSize = shieldName?.toLowerCase().includes("large") ? "large" : 
                     shieldName?.toLowerCase().includes("small") ? "small" : "medium";
  ...
  return (
    ...
    >
      ...
      <div className="absolute -top-8 left-1/2 -translate-x-1/2 w-24">
        ...
        {/* Winner glow */}
        {isWinner && (
          <div className="absolute -inset-1 bg-yellow-500/30 blur-sm animate-pulse" />
        )}
      </div>
      ...
      {/* Fighter SVG */}
      <div className={cn("relative", stanceAnimation, idleAnimation)} style={{ width: size, height: size * 1.5 }}>
        <svg
          viewBox="0 0 100 150"
          fill="none"
        ...
        >
          ...
          <g className={cn("transition-transform duration-200", isDead && "translate-y-8")}>
            ...
            <circle
              ...
              r="10"
              className={cn(
                "fill-amber-200/80 stroke-amber-900/40",
                isDead && "fill-gray-400/80"
              )}
              strokeWidth="2"
            />
            ...
            <path
              ...
              className={cn(
                "fill-amber-800/60 stroke-amber-900/40",
                isDead && "fill-gray-600/60"
              )}
              strokeWidth="2"
            />
            ...
            <path
              ...
              className={cn(
                "fill-amber-700/50 stroke-amber-900/40",
                isDead && "fill-gray-700/50"
              )}
              strokeWidth="2"
            />
            ...
            <path
              ...
              className={cn(
                "fill-amber-200/70 stroke-amber-900/40",
                isDead && "fill-gray-400/70"
              )}
              strokeWidth="2"
            />
            ...
            <path
              ...
              className={cn(
                "fill-amber-700/60 stroke-amber-900/40",
                isDead && "fill-gray-600/60"
              )}
              strokeWidth="2"
            />
...
function WeaponIcon({ category, pose, x, y }: { category: string; pose: string; x: number; y: number }) {
  ...
  if (category === "bash") {
    return (
      <g transform={transform}>
        <circle cx={x} cy={y - 5} r="10" className="fill-slate-500/90 stroke-slate-700" strokeWidth="2" />
  ...
  return (
    <g transform={transform}>
      <circle cx={x} cy={y - 5} r="6" className="fill-amber-200/80 stroke-amber-900/40" strokeWidth="2" />
    </g>
  );
}
