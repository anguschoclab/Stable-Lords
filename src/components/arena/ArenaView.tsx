...
export default function ArenaView({
  ...
  const [weaponSwing, setWeaponSwing] = useState<{active: boolean; fighter: "A" | "D" | null}>({active: false, fighter: null});
  ...
  const getWeaponCategory = (gear?: Gear): "slash" | "bash" | "pierce" | "fist" => {
    ...
    if (name.includes("sword") || name.includes("scimitar") || name.includes("axe") || name.includes("blade")) return "slash";
    if (name.includes("mace") || name.includes("hammer") || name.includes("flail") || name.includes("maul")) return "bash";
    if (name.includes("spear") || name.includes("dagger") || name.includes("épée") || name.includes("epee")) return "pierce";
    return "fist";
  };
  ...
  return (
    <ScreenShake 
      ...
      intensity={arenaPrefs.screenShakeIntensity === "off" ? "low" : arenaPrefs.screenShakeIntensity}
