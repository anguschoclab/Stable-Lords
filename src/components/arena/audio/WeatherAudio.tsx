...
export default function WeatherAudio({ weather, volume, enabled }: WeatherAudioProps) {
  ...
  useEffect(() => {
    ...
    if (weather !== currentWeatherRef.current) {
      const oldAmbience = currentWeatherRef.current ? WEATHER_AMBIENCE[currentWeatherRef.current] : null;
      const newAmbience = WEATHER_AMBIENCE[weather];
      
      if (oldAmbience !== newAmbience) {
        console.log(`[WeatherAudio] Crossfade: ${oldAmbience || "none"} → ${newAmbience || "none"} at ${volume * 100}%`);
