...
export default function CrowdReactions({ tier, state, className }: CrowdReactionsProps) {
  ...
  const getCrowdAnimation = () => {
    switch (currentState) {
      case "cheer": return "crowd-cheer";
      case "roar": return "crowd-cheer";
      case "gasp": return "crowd-gasp";
      case "chant": return "crowd-chant";
      default: return "";
    }
  };
  ...
  const getOpacity = () => {
    switch (currentState) {
      case "anticipation": return 0.3;
      case "silence": return 0.15;
      default: return tier === "grand" ? 0.5 : tier === "championship" ? 0.4 : 0.25;
    }
  };

  return (
    <div className={cn("absolute top-0 left-0 right-0 h-1/4 overflow-hidden pointer-events-none", className)}>
      <svg 
        viewBox="0 0 100 25" 
