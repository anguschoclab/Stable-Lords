...
export function DeathModal() {
  const { unacknowledgedDeaths, graveyard, acknowledgeDeath } = useGameStore(
    useShallow((s) => ({
      unacknowledgedDeaths: s.state.unacknowledgedDeaths,
      graveyard: s.state.graveyard,
      acknowledgeDeath: (id: string) => {
        const next = s.state.unacknowledgedDeaths.filter(d => d !== id);
        s.setState({ ...s.state, unacknowledgedDeaths: next });
      }
    }))
  );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50">Service</span>
                ...
                <div className="space-y-1">
                  <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50">Legacy</span>
                  <div className="text-sm font-mono flex items-center gap-2">
                    <Crosshair className="w-3 h-3 text-arena-blood" />
                    <span>{warrior.career.wins}W - {warrior.career.losses}L</span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex justify-center bg-black/40 rounded-xl p-4 border border-white/5 relative group">
                ...
                <div className="relative w-full max-w-xs aspect-[1/2] opacity-60 grayscale filter contrast-125">
                   <PaperDoll healthMap={{}} />
                </div>
            </div>
          </div>
