...
export function DominantCombatantsSection({ topWarriorA, topWarriorB }: DominantCombatantsSectionProps) {
  return (
    <Surface variant="glass" padding="none" className="border-border/40 overflow-hidden">
      ...
      <div className="p-6 grid grid-cols-2 gap-8">
        {[
          { warrior: topWarriorA, color: 'text-primary', borderColor: 'border-primary/20', bgColor: 'bg-primary/5' },
          { warrior: topWarriorB, color: 'text-accent', borderColor: 'border-accent/20', bgColor: 'bg-accent/5' },
        ].map(({ warrior, color, borderColor, bgColor }) => (
          <div key={warrior.id} className={`p-4 border rounded-none ${borderColor} ${bgColor}`}>
            <div className="space-y-3">
              <div className={`text-[9px] font-black uppercase tracking-widest mb-3 opacity-60 ${color}`}>
                Top Warrior
              </div>
