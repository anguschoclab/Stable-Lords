...
export function EditableText({ value, onSave, className, inputClassName, label }: EditableTextProps) {
  ...
  if (isEditing) {
    return (
      <div className={cn("flex items-center gap-2 animate-in fade-in zoom-in-95 duration-200", className)}>
        <Input
          ref={inputRef}
          value={tempValue}
        ...
        <div className="flex items-center gap-1">
           <Button size="icon" variant="ghost" className="h-8 w-8 text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/10" onClick={handleSave}>
              <Check className="h-4 w-4" />
           </Button>
           <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive hover:text-destructive/80 hover:bg-destructive/10" onClick={handleCancel}>
              <X className="h-4 w-4" />
           </Button>
        </div>
      </div>
