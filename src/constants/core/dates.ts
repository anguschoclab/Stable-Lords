...
/**
 * Compute a UTC timestamp (ISO string) for a given game week, anchored to ERA_START_YEAR.
 * Week 1 → Jan 1 of ERA_START_YEAR. Week N → Jan 1 + (N-1)*7 days.
 */
export function weekToTimestamp(week: number): string {
  return new Date(
    Date.UTC(ERA_START_YEAR, 0, 1) + (week - 1) * MS_PER_WEEK
  ).toISOString();
}
