...
function getQuicknessStatement(defBase: number, parBase: number, wt: number): string {
  ...
  if (defBase >= 35) {
    ...
    qualifier = parBase >= 30 ? "making even dangerous opponents look harmless" : parBase <= 0 ? "relying on speed to stay out of danger" : "";
  } else if (defBase >= 25) {
    speed = "is very quick on his feet";
    qualifier = parBase >= 30 ? "often avoiding seemingly hopeless situations" : parBase <= 0 ? "avoiding rather than trading blows" : "";
  } else if (defBase >= 15) {
    speed = "is quick on his feet";
    qualifier = parBase >= 30 ? "and is well able to protect himself" : parBase <= 0 ? "avoiding blows rather than parrying" : "";
  } else if (defBase >= 5) {
    return "";
  } else if (defBase >= -10) {
    speed = "is slow on his feet";
    qualifier = parBase >= 30 ? "finding it hard to avoid a blow he doesn't parry" : parBase <= 0 ? "with a marked inability to avoid or parry a blow" : "";
  } else {
    speed = "is very slow on his feet";
    qualifier = parBase >= 30 ? "attempts to parry rather than dodge attacks" : parBase <= 0 ? "absolutely unable to avoid getting hurt" : "";
  }
