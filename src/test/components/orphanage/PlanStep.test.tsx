...
describe('PlanStep — sliders', () => {
  it('renders "Offensive Effort" label', () => {
    render(<PlanStep warrior={warrior} plan={plan} onPlanChange={vi.fn()} onBack={vi.fn()} onNext={vi.fn()} />);
  ...
  it('displays current OE value', () => {
    const p = { ...plan, OE: 7 };
    render(<PlanStep warrior={warrior} plan={p} onPlanChange={vi.fn()} onBack={vi.fn()} onNext={vi.fn()} />);
...
describe('PlanStep — tactic buttons', () => {
  ...
  it('clicking offensive tactic calls onPlanChange with offensiveTactic set', () => {
    ...
    render(<PlanStep warrior={warrior} plan={plan} onPlanChange={onPlanChange} onBack={vi.fn()} onNext={vi.fn()} />);
    fireEvent.click(screen.getByText('Slash'));
    expect(onPlanChange).toHaveBeenCalledWith(expect.objectContaining({ offensiveTactic: 'Slash' }));
  });
  ...
  it('clicking defensive tactic calls onPlanChange with defensiveTactic set', () => {
    ...
    expect(onPlanChange).toHaveBeenCalledWith(expect.objectContaining({ defensiveTactic: 'Riposte' }));
  });
...
describe('PlanStep — navigation', () => {
  it('clicking "To the Arena" calls onNext', () => {
    const onNext = vi.fn();
    render(<PlanStep warrior={warrior} plan={plan} onPlanChange={vi.fn()} onBack={vi.fn()} onNext={onNext} />);
  ...
  it('clicking Back calls onBack', () => {
    const onBack = vi.fn();
    render(<PlanStep warrior={warrior} plan={plan} onPlanChange={vi.fn()} onBack={onBack} onNext={vi.fn()} />);
