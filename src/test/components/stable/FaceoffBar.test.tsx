...
describe('FaceoffBar', () => {
  it('renders without crashing', () => {
    const { container } = render(<FaceoffBar hpA={80} maxA={100} hpB={60} maxB={100} />);
  ...
  it('shows 0 width when hpA is 0 (knocked out)', () => {
    const { container } = render(<FaceoffBar hpA={0} maxA={100} hpB={60} maxB={100} />);
    const fillA = container.querySelector('[data-testid="faceoff-fill-a"]') as HTMLElement;
    expect(fillA.style.width).toBe('0%');
  });

  it('clamps hpA below 0 to 0 width', () => {
    const { container } = render(<FaceoffBar hpA={-10} maxA={100} hpB={60} maxB={100} />);
    const fillA = container.querySelector('[data-testid="faceoff-fill-a"]') as HTMLElement;
    expect(fillA.style.width).toBe('0%');
  });
});
