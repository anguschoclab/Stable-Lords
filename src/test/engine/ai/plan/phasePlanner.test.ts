...
describe('phasePlanner', () => {
  ...
  describe('buildPhasePlan', () => {
    it('builds phase plan with conservative opening for balanced style', () => {
      const phases = buildPhasePlan(basePlan, 'Balanced' as any, FightingStyle.StrikingAttack);

      // Opening
      expect((phases as any).opening.OE).toBe(4); // 5 - 1
      expect((phases as any).opening.AL).toBe(5);
      expect((phases as any).opening.killDesire).toBe(4);
      expect((phases as any).opening.aggressionBias).toBe(4); // 5 - 1

      // Mid
      expect((phases as any).mid.OE).toBe(5);
      expect((phases as any).mid.AL).toBe(5);
      expect((phases as any).mid.killDesire).toBe(5);
      expect((phases as any).mid.aggressionBias).toBe(5);

      // Late
      expect((phases as any).late.OE).toBe(5);
      expect((phases as any).late.AL).toBe(5);
      expect((phases as any).late.killDesire).toBe(5);
      expect((phases as any).late.aggressionBias).toBe(5);
    });

    it('builds phase plan with very conservative opening for defensive style', () => {
      const phases = buildPhasePlan(basePlan, 'Balanced' as any, FightingStyle.TotalParry);

      // Opening
      expect((phases as any).opening.OE).toBe(3); // 5 - 2
      expect((phases as any).opening.AL).toBe(5);
    });
    ...
    it('scales up aggression late for Aggressive personalities', () => {
      ...
      // Late
      expect((phases as any).late.OE).toBe(6); // 5 + 1
      expect((phases as any).late.AL).toBe(5);
      expect((phases as any).late.aggressionBias).toBe(6); // 5 + 1
    });
    ...
    it('scales up aggression late for Showman personalities', () => {
      ...
      // Late
      expect((phases as any).late.OE).toBe(6);
      expect((phases as any).late.aggressionBias).toBe(6);
    });
    ...
    it('scales down aggression and up AL late for Methodical personalities', () => {
      ...
      // Late
      expect((phases as any).late.OE).toBe(4); // 5 - 1
      expect((phases as any).late.AL).toBe(6); // 5 + 1
      expect((phases as any).late.aggressionBias).toBe(5); // base
    });
    ...
    it('scales down aggression and up AL late for Tactician personalities', () => {
      ...
      // Late
      expect((phases as any).late.OE).toBe(4);
      expect((phases as any).late.AL).toBe(6);
    });
    ...
    it('clamps OE/AL correctly at boundaries', () => {
      ...
      // Late
      expect((phases as any).late.OE).toBe(10); // Not 11
      expect((phases as any).late.AL).toBe(10);
      expect((phases as any).late.aggressionBias).toBe(10); // Not 11
    });
  });
  ...
  describe('buildDesperatePlan', () => {
    it('returns hyper-aggressive desperate plan for Aggressive', () => {
      ...
      expect((plan as any).OE).toBe(4); // base 5 - 1
      expect((plan as any).AL).toBe(6); // base 5 + 1
      expect((plan as any).killDesire).toBe(6); // base 5 + 1
    });

    it('returns hyper-defensive desperate plan for Methodical', () => {
      const plan = buildDesperatePlan(basePlan, 'Methodical');
      expect((plan as any).OE).toBe(1); // hardcoded 1
      expect((plan as any).AL).toBe(8); // base 5 + 3
      expect((plan as any).killDesire).toBe(3); // base 5 - 2
    });

    it('returns generic desperate plan for other personalities', () => {
      const plan = buildDesperatePlan(basePlan, 'Balanced' as any);
      expect((plan as any).OE).toBe(3); // base 5 - 2
      expect((plan as any).AL).toBe(7); // base 5 + 2
      expect((plan as any).killDesire).toBe(3); // base 5 - 2
    });
    ...
    it('clamps values correctly', () => {
      ...
      expect((plan as any).OE).toBe(1); // Not 0
      expect((plan as any).AL).toBe(10); // Not 11
      expect((plan as any).killDesire).toBe(10); // Not 11
    });
  });
