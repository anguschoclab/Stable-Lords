...
describe('absoluteWeek rollover — convertBidsToOffers', () => {
  it('produces boutWeek = 53 at year boundary (absoluteWeek=52, year=1)', () => {
    ...
    expect(offers[0]!.boutWeek).toBe(53);
    expect(offers[0]!.expirationWeek).toBe(53);
  });

  it('produces boutWeek = 105 at year 2 boundary (absoluteWeek=104, year=2)', () => {
    const wA = makeWarrior('wA');
    const wB = makeWarrior('wB');
    const rivalA = makeRival('rA', [wA]);
    const rivalB = makeRival('rB', [wB]);
    ...
    expect(offers.length).toBeGreaterThan(0);
    expect(offers[0]!.boutWeek).toBe(105);
    expect(offers[0]!.expirationWeek).toBe(105);
  });
});

describe('absoluteWeek rollover — generatePairings', () => {
  it('finds offers with boutWeek = 53 when state.absoluteWeek = 53', () => {
    const wA = makeWarrior('wA');
    const wB = makeWarrior('wB');
    const rivalA = makeRival('rA', [wA]);
    const rivalB = makeRival('rB', [wB]);
    ...
    // Generate offers at absoluteWeek=52 → boutWeek=53
    const { bids } = generateBoutBids(rivalA, 52, 'Clear', 'Calm', [rivalB]);
    const rng = new SeededRNGService(42);
    const allBids = bids.map((bid) => ({ bid, rivalId: rivalA.id as string }));
    const offers = convertBidsToOffers(allBids, [rivalA, rivalB], state, rng, new Set());
    ...
    // Advance to absoluteWeek=53 (year 2, week 1)
    const stateAt53 = {
      ...state,
      absoluteWeek: 53,
      week: 1,
    ...
    const pairings = generatePairings(stateAt53);
