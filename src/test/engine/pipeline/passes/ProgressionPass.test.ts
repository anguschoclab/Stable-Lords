...
describe('ProgressionPass', () => {
  ...
  describe('Stable standing computation', () => {
    it('player highest fame → standing 1', () => {
      state.fame = 999;
      state.rivals.forEach((r) => { r.fame = 100; });
    ...
    it('player mid fame → correct rank', () => {
      ...
      state.rivals.slice(5).forEach((r) => { r.fame = 10; });
    ...
    it('deterministic tie-break — same fame → titles → stableId', () => {
      ...
      state.rivals = [
        {
          id: 'rival_a' as any,
          owner: { id: 'rival_a' as any, name: 'A', stableName: 'A', fame: 100, renown: 0, titles: 3 },
          fame: 100, roster: [], treasury: 0, ledger: [], trainingAssignments: [],
        },
        {
          id: 'rival_z' as any,
          owner: { id: 'rival_z' as any, name: 'Z', stableName: 'Z', fame: 100, renown: 0, titles: 5 },
          fame: 100, roster: [], treasury: 0, ledger: [], trainingAssignments: [],
        },
      ];
  ...
  describe('TOP_10_STABLE', () => {
    it('completes when standing <= 10', () => {
      state.fame = 500;
      state.rivals.forEach((r, i) => { r.fame = i < 5 ? 600 : 50; });
  ...
  describe('TOP_3_STABLE', () => {
    ...
    it('does NOT complete when standing > 3', () => {
      state.fame = 50;
      state.rivals.forEach((r, i) => { r.fame = 200 + i * 10; });
  ...
  describe('FIRST_TOURNAMENT_WIN', () => {
    ...
    it('does NOT complete when champion is rival warrior', () => {
      const playerWarrior = makeWarrior('w1' as WarriorId, 'Player One', FightingStyle.StrikingAttack, {
        ST: 10, CN: 10, SZ: 10, WT: 10, WL: 10, SP: 10, DF: 10,
      });
  ...
  describe('REALM_CHAMPION', () => {
    ...
    it('does NOT complete at year boundary if standing !== 1', () => {
      state.fame = 50;
      state.rivals.forEach((r) => { r.fame = 200; });
