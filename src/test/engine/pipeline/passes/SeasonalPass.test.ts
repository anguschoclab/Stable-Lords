...
describe('runSeasonalPass', () => {
  ...
  it('should trigger the underground_pit_fight offseason event', () => {
    const rng = new SeededRNGService(99);
    const originalNext = rng.next.bind(rng);
    let callCount = 0;
    const mockNext = () => {
      callCount++;
      if (callCount === 1) return 18.5 / 19; // picks index 18 = underground_pit_fight
      return originalNext();
    };
    rng.next = mockNext;

    const warriorId = 'w-pit-fight' as WarriorId;
    const state: Partial<GameState> = {
      year: 1,
      roster: [{ id: warriorId, name: 'Brawler', status: 'Active', fame: 10, injuries: [] } as any],
    };
    ...
    const update = impact.rosterUpdates!.get(warriorId)!;
    expect(update.fame).toBeGreaterThan(10);
    expect(update.injuries).toHaveLength(1);
    expect(update.injuries![0].name).toBe('Busted Knuckles');
  });
});
