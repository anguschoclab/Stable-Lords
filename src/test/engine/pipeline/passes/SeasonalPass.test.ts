...
describe('runSeasonalPass', () => {
  ...
  it('should trigger the black_market_raid offseason event, deduct gold, and newsletter', () => {
    ...
    const originalNext = (rng as any).next.bind(rng);
    let callCount = 0;
    const mockNext = () => {
      callCount++;
  ...
  it('should trigger the meteor_shower offseason event and award xp and fame', () => {
    ...
    const mockNext = () => {
      callCount++;
      if (callCount === 1) return 18.5 / 19; // picks index 18 = meteor_shower
      return originalNext();
    };
    (rng as any).next = mockNext;
    ...
    const state: Partial<GameState> = {
      year: 1,
      roster: [{ id: warriorId, name: 'Stargazer', status: 'Active', xp: 0, fame: 0 } as any],
      newsletter: [],
    };
    ...
    const wUpdate = impact.rosterUpdates?.get(warriorId);
    expect(wUpdate).toBeDefined();
    expect(wUpdate?.xp).toBeGreaterThanOrEqual(15);
    expect(wUpdate?.xp).toBeLessThanOrEqual(25);
    expect(wUpdate?.fame).toBeGreaterThanOrEqual(10);
    expect(wUpdate?.fame).toBeLessThanOrEqual(15);

    expect(impact.newsletterItems).toHaveLength(1);
    expect(impact.newsletterItems?.[0]?.title).toBe('Meteor Shower');
    expect(impact.newsletterItems?.[0]?.items[0]).toContain('A stunning meteor shower lit up the night sky!');
  });
});
