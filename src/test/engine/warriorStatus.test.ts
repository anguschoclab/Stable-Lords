...
describe('isBookable', () => {
  ...
  describe('status checks', () => {
    it('returns true for active, healthy, non-resting, non-training warrior', () => {
      expect(
        isBookable(makeBookableWarrior(), {
          restStates: noRest,
          trainingAssignments: noTraining,
          targetWeek,
        })
      ).toBe(true);
    });
    ...
    it('returns false for dead warrior (status === Dead)', () => {
      expect(
        isBookable(makeBookableWarrior('Dead'), {
          restStates: noRest,
          trainingAssignments: noTraining,
          targetWeek,
        })
    ...
    it('returns false for retired warrior', () => {
      expect(
        isBookable(makeBookableWarrior('Retired'), {
          restStates: noRest,
          trainingAssignments: noTraining,
          targetWeek,
        })
    ...
    it('returns false for warrior with isDead: true property even if status is Active', () => {
      expect(
        isBookable(makeBookableWarrior('Active', [], true), {
          restStates: noRest,
          trainingAssignments: noTraining,
          targetWeek,
        })
  ...
  describe('rest gate', () => {
    it('returns false for warrior with active rest state (restUntilWeek > targetWeek)', () => {
      const restStates: RestState[] = [
        { warriorId: 'test-warrior' as WarriorId, restUntilWeek: 12 },
      ];
      expect(
        isBookable(makeBookableWarrior(), {
    ...
    it('returns true for warrior with expired rest state (restUntilWeek <= targetWeek)', () => {
      const restStates: RestState[] = [
        { warriorId: 'test-warrior' as WarriorId, restUntilWeek: 10 },
      ];
      expect(
        isBookable(makeBookableWarrior(), {
          restStates: restStates,
    ...
    it('returns true when targetWeek is beyond rest period', () => {
      const restStates: RestState[] = [
        { warriorId: 'test-warrior' as WarriorId, restUntilWeek: 5 },
      ];
      expect(
        isBookable(makeBookableWarrior(), {
          restStates: restStates,
    ...
    it('returns true when rest state is for a different warrior', () => {
      const restStates: RestState[] = [
        { warriorId: 'other-warrior' as WarriorId, restUntilWeek: 20 },
      ];
      expect(
        isBookable(makeBookableWarrior(), {
          restStates: restStates,
  ...
  describe('injury gate', () => {
    it('returns false for warrior with severe injury (>2 weeks remaining)', () => {
      expect(
        isBookable(makeBookableWarrior('Active', [makeInjury('Severe', 5)]), {
          restStates: noRest,
          trainingAssignments: noTraining,
          targetWeek,
        })
    ...
    it('returns true for warrior with moderate injury (not too injured to fight)', () => {
      expect(
        isBookable(makeBookableWarrior('Active', [makeInjury('Moderate', 4)]), {
          restStates: noRest,
          trainingAssignments: noTraining,
          targetWeek,
        })
    ...
    it('returns true for warrior with severe injury (<=2 weeks remaining)', () => {
      expect(
        isBookable(makeBookableWarrior('Active', [makeInjury('Severe', 2)]), {
          restStates: noRest,
          trainingAssignments: noTraining,
          targetWeek,
        })
  ...
  describe('training gate', () => {
    it('returns false for warrior with active training assignment', () => {
      const training: TrainingAssignment[] = [
        { warriorId: 'test-warrior' as WarriorId, type: 'trait', weeksRemaining: 3 },
      ];
      expect(
        isBookable(makeBookableWarrior(), {
          restStates: noRest,
    ...
    it('returns true when training assignment is for a different warrior', () => {
      const training: TrainingAssignment[] = [
        { warriorId: 'other-warrior' as WarriorId, type: 'trait', weeksRemaining: 3 },
      ];
      expect(
        isBookable(makeBookableWarrior(), {
          restStates: noRest,
  ...
  describe('combined gates', () => {
    it('returns false when multiple gates fail simultaneously (rest + injury + training)', () => {
      const restStates: RestState[] = [
        { warriorId: 'test-warrior' as WarriorId, restUntilWeek: 20 },
      ];
      const training: TrainingAssignment[] = [
        { warriorId: 'test-warrior' as WarriorId, type: 'trait', weeksRemaining: 3 },
      ];
