...
describe('calculateArenaLeaderboard', () => {
  it('returns empty array for empty roster and no rivals', () => {
    const result = calculateArenaLeaderboard([], undefined, 'Player Stable');
  ...
  it('mixes player and rival warriors in a single descending-fame list', () => {
    ...
    const result = calculateArenaLeaderboard(roster, rivals, 'Player Stable');
    expect(result).toHaveLength(3);
    expect(result[0]!.warrior.id).toBe('r-high');
  ...
  it('sets isPlayer: true for player-roster entries and false for rival entries', () => {
    ...
    const result = calculateArenaLeaderboard(roster, rivals, 'Player Stable');
    expect(result.find((e) => e.warrior.id === 'p1')!.isPlayer).toBe(true);
    expect(result.find((e) => e.warrior.id === 'r1')!.isPlayer).toBe(false);
  });
  ...
  it('maps playerStableName for player entries and rival.owner.stableName for rival entries', () => {
    ...
    const result = calculateArenaLeaderboard(roster, rivals, 'Blood Hawks');
    expect(result.find((e) => e.warrior.id === 'p1')!.stableName).toBe('Blood Hawks');
    expect(result.find((e) => e.warrior.id === 'r1')!.stableName).toBe('Iron Skulls');
  });
  ...
  it('respects custom limit', () => {
    ...
    const result = calculateArenaLeaderboard(roster, undefined, 'Player Stable', 3);
    expect(result).toHaveLength(3);
  ...
  it('handles limit = 0', () => {
    ...
    const result = calculateArenaLeaderboard(roster, undefined, 'Player Stable', 0);
    expect(result).toEqual([]);
  });
  ...
  it('drops late-arriving equal-fame warriors at the capacity boundary', () => {
    ...
    const result = calculateArenaLeaderboard(roster, undefined, 'Player Stable', 2);
    expect(result).toHaveLength(2);
